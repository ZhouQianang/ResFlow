...
add_smooth_noise = SmoothNoise()

class TDF_noscale(nn.Module):
    def __init__(self, 
                input_bins=15, 
                noise_weight=0.3,
                param_frozen=False,
                ckpt_path=None):
        super(TDF_noscale, self).__init__()
    ...
    def forward(self, x1, x2, iters=6, diters=2, add_noise=False):
...
#             flow_result.update({'global_pred_list': flow_predictions})
        ...
        if add_noise and self.training:
            flow, _ = add_smooth_noise.add_noise(flow, self.noise_weight)
            # print("add noise!!!!")
        
        flows_dense = [flow*0.2, flow*0.4, flow*0.6, flow*0.8, flow]
        ...
        for iter in range(diters):
            ...
            for i in range(self.split):
                coords = (coords0 + flows_dense[i]).detach()
                corr_map = corr_fn_list[i](coords)
                corr_map_list.append(corr_map)
            ...
            if self.training:
                ...
                flows_scaled = torch.cat([flows[i] for i in range(len(flows))], dim=0)
