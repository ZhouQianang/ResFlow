...
# ######## CMA for dense pred
class CMA2(nn.Module):
    def __init__(self,
                d_model=128):
        super(CMA2, self).__init__()
        self.d_model = d_model
        self.layer1 = AttnLayerNoV(d_model, mid_dim=d_model)
    ...
    def forward(self, mfs):
        ...
        concat0 = torch.cat(mfs[:-1], dim=0).flatten(-2).permute(0, 2, 1)
        concat1 = torch.cat(mfs[-1:]*(t-1), dim=0).flatten(-2).permute(0, 2, 1)
        ...
        return concat0.chunk(t-1, dim=0)
    
    
######## CMA for dense pred of TMA
# class CMA2(nn.Module):
#     def __init__(self,
#                 d_model=128):
#         super(CMA2, self).__init__()
...
#         concat0 = torch.cat(mfs, dim=0).flatten(-2).permute(0, 2, 1)
#         concat1 = torch.cat(mfs[-1:]*t, dim=0).flatten(-2).permute(0, 2, 1)
